...
public class DateUtils {
   ...
   * @return
   */
  public static boolean isTimeInterval(Date startTime, Date endTime, Date nowTime) {
    startTime = getDateTime(startTime);
    endTime = getDateTime(endTime);
    nowTime = getDateTime(nowTime);
  ...
  /**
   * 判断当前时间是否在两个时间段内，如果startTime="23:30:00" ，endTime="06:00:00", 当前时间为
   * nowTime=00:00:00，那么当前时间nowTime也是区间内的。 传入的时间格式为 yyyy-MM-dd
   * HH:mm:ss，即直接传入一个标准时间即可，无需自己处理时间格式，本方法会自行处理。
   * 
   * @param startDate
   * @param endDate
   * @param nowDate
   * @return
   */
  public static boolean isTimeInterior(Date startDate, Date endDate, Date nowDate) {
    ...
    String now = simpleDateFormat.format(nowDate);// 当前时间
    String mStartTime = simpleDateFormat.format(startDate);// 开始时间
    String mEndTime = simpleDateFormat.format(endDate);// 结束时间
    boolean isTrue = false;
    try {
      Date nowTime = simpleDateFormat.parse(now);
      Date start = simpleDateFormat.parse(mStartTime);
      Date end = simpleDateFormat.parse(mEndTime);

      int is = start.compareTo(end);

      if (is == 1) {
        // 构造一个 23:59:59 时间
        Date eTime = simpleDateFormat.parse(LATE_TIME);
        Date sTime = simpleDateFormat.parse(EARLY_TIME);
        boolean TheFirstTime = isTimeInterval(start, eTime, nowTime);
        boolean secondTime = isTimeInterval(sTime, end, nowTime);

        if (TheFirstTime || secondTime) {
          isTrue = true;
        }
      } else if (is == 0) {
        isTrue = true;
      } else {
        isTrue = isTimeInterval(startDate, endDate, nowDate);
      }
    } catch (ParseException e) {
      e.printStackTrace();
    }
