...
(function($, window, document, undefined) {
			...
			$.fn.lazyload = function(options) {
				...
				function update() {
					...
					elements.each(function() {
								...
								if ($.abovethetop(this, settings)
									...
									$this.trigger("appear");
									/*
									 * if we found an image we'll load, reset
									 * the counter
									 */
									counter = 0;
								} else {
					...
					$self.one("appear", function() {
								if (!this.loaded) {
									...
									$("<img />").bind("load", function() {
										...
										/*
										 * Remove image from array so it is not
										 * looped next time.
										 */
										var temp = $.grep(elements, function(
														element) {
				...
				/*
				 * With IOS5 force loading images when navigating with back
				 * button.
				 */
			...
			/*
			 * Use as $.belowthefold(element, {threshold : 100, container :
			 * window})
			 */
			...
			$.belowthefold = function(element, settings) {
				...
				if (settings.container === undefined
					...
					fold = (window.innerHeight ? window.innerHeight : $window
							.height())
							+ $window.scrollTop();
				} else {
