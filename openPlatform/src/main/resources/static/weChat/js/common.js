...
define(function(require, exports, module) {
	...
	/** 获取url后面的参数 */
	exports.getUrlParam = function(url) {
		console.log(url)
		var u = url.split("?");
	...
	/** 判读是否传入的字符串str是否包含include */
	exports.isInclude = function(str, include) {
	 ...
	 */
	exports.formatDate = function(nowDate, format) {
		var nowStr = null;
	 ...
	 *            日期转为字符串时的格式化参数
	 */
	exports.get7DaysBefore = function(date, strFlag, format) {
		var days = [];
		...
		if (strFlag) {
			days.unshift(this.formatDate(date, format));
		} else {
			days.unshift(date);
		}
		timestamp = date.getTime();
		for (var i = 1; i <= 7; i++) {
			var tempDay = new Date(timestamp - i * 24 * 3600 * 1000);
			if (strFlag) {
				days.unshift(this.formatDate(tempDay, format));
			} else {
				days.unshift(tempDay);
			}
		}
		return days;
	}
	 ...
	 *            日期转为字符串时的格式化参数
	 */
	exports.get4WeekBefore = function(date, format) {
		var days = [];
		var date = date || new Date(), timestamp, newDate;
		if (!(date instanceof Date)) {
			date = new Date(date.replace(/-/g, '/'));
		...
		var first = date.setDate(date.getDate() - num); // 本周第一天
		var last = date.setDate(date.getDate() + 6);// 本周最后一天
		days.unshift(this.formatDate(new Date(first), format) + "-"
				+ this.formatDate(new Date(last), format));
		for (var i = 1; i <=4; i++) {
			first = new Date(first -  7 * 24 * 3600 * 1000);
			last = new Date(last -  7 * 24 * 3600 * 1000);
			days.unshift(this.formatDate(new Date(first), format) + "-"
					+ this.formatDate(new Date(last), format));
		}
		return days;
	 ...
	 *            日期转为字符串时的格式化参数
	 */
	exports.getWeekBefore = function(date, format) {
