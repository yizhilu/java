...
@Service("XicidailiCrawleService")
public class XicidailiCrawleService extends BreadthCrawler implements Runnable {
  private static final Logger LOGGER = LoggerFactory.getLogger(XicidailiCrawleService.class);
  @Autowired
  private ProxyDataRepository proxyDataRepository;
  @Autowired
  private DynamicTaskJobService dynamicTaskJobService;
  public XicidailiCrawleService() {
    super("crawl", true);

  }

  public XicidailiCrawleService(String crawlPath, boolean autoParse) {
  ...
  public void visit(Page page, CrawlDatums next) {
    ...
    // 搜索
    if (Pattern.matches("^http://www.xicidaili.com/nn.*$", pageUrl)) {
      analysis(page, next);
    }
  ...
  private void analysis(Page page, CrawlDatums next) {
    ...
    driver.get(page.url());
    List<WebElement> trs = driver.findElements(By.xpath("//table[@id='ip_list']/tbody/tr"));
    if (trs.size() > 0) {
      List<ProxyDataEntity> proxyDatas = proxyDataRepository
          .findByDataSource("http://www.xicidaili.com", DateUtils.getEarlyInTheDay(new Date()));
    ...
    for (WebElement tr : trs) {
      ...
      String ip = tds.get(1).getText();
      String port = tds.get(2).getText();
      String address = tds.get(3).getText();
      String anonymousType = tds.get(4).getText();
      String type = tds.get(5).getText();
      ...
      if ("HTTP".equals(type) || "HTTPS".equals(type)) {
        ...
        proxyData.setDataSource("http://www.xicidaili.com");
  ...
  public static void main(String[] args) throws Exception {
    XicidailiCrawleService crawleService = new XicidailiCrawleService();

    crawleService.addSeed("http://www.xicidaili.com/nn");
  ...
  public void run() {
    ...
    this.addSeed("http://www.xicidaili.com/nn");
